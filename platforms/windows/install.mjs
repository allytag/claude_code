...
const TASK_TEMPLATE = path.join(HERE, "task-scheduler", "openrouter-claude-proxy.xml.template");
...
const WRAPPER_FILES = [
  ...
  "claude-qwen.cmd",
  "claude-deepseek.cmd",
  "start-openrouter-claude-proxy.cmd",
];

function usage() {
  return `Usage: node platforms/windows/install.mjs [--dry-run] [--merge|--fresh|--upgrade] [--yes]

Windows support is beta until verified on a real Windows machine.

Options:
  --dry-run              Print preflight + write plan only
  --merge                Merge with existing %USERPROFILE%\\.claude\\settings.json (default)
  --fresh                Refuse if %USERPROFILE%\\.claude is non-empty
  --upgrade              Same behavior as merge, labels intent as upgrade
  --yes                  Skip confirmation prompt
  --api-key-env NAME     Read OpenRouter key from env var NAME
  --enable-task          Create/update a Task Scheduler task for proxy startup
  --fake-home PATH       Test-only target home for dry-run validation
  --skip-platform-check  Test-only; lets static validation run on non-Windows
  -h, --help             Show help
`;
}
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (arg === "--enable-task") opts.enableTask = true;
...
async function findCommand(name) {
  if (process.platform === "win32") {
    const out = await run("where", [name]);
...
function targets(home, fakeHome) {
  const appData = fakeHome ? path.join(home, "AppData", "Roaming") : (process.env.APPDATA || path.join(home, "AppData", "Roaming"));
  return {
    home,
    ...
    taskXml: path.join(home, ".claude", "openrouter-claude-proxy", "openrouter-claude-proxy.task.xml"),
  };
}
...
async function preflight(opts, t) {
  ...
  const schtasksPath = await findCommand("schtasks");
  ...
  const report = {
    repo: REPO,
    status: "windows-beta-static-validated",
    mode: opts.mode,
    dryRun: opts.dryRun,
    ...
    vscode: { codePath, version: codeVersion.stdout.split(/\r?\n/)[0] || null, settingsPath: t.vscodeSettings, optional: true },
    taskScheduler: { schtasksPath, requested: opts.enableTask, ok: Boolean(schtasksPath) },
  ...
  if (process.platform !== "win32" && !opts.skipPlatformCheck) report.blockers.push("Windows required. Use macOS/Linux installer on other platforms.");
  ...
  if (!codePath) report.warnings.push("VS Code CLI `code` not found. VS Code settings will still be written to AppData path.");
  if (opts.enableTask && !schtasksPath) report.blockers.push("Task Scheduler command `schtasks` not found");
  if (!opts.enableTask) report.warnings.push("Task Scheduler not enabled. Start proxy with start-openrouter-claude-proxy.cmd or rerun with --enable-task.");
  if (opts.mode === "fresh" && report.existingClaudeDir.exists) report.blockers.push("--fresh requires empty or absent %USERPROFILE%\\.claude");
  report.ok = report.blockers.length === 0;
  return report;
}

function renderPayload(text, t) {
  return text
    .replaceAll("__HOME__", slashPath(t.home))
    .replaceAll("__NODE__", slashPath(process.execPath));
}

function renderCmd(text, t) {
  return text
    .replaceAll("__HOME__", cmdPath(t.home))
    .replaceAll("__NODE__", cmdPath(process.execPath));
}
...
async function buildPlan(opts, t) {
  ...
  for (const name of REQUIRED_PROXY_FILES) {
    ...
    plan.push({ action: "write", from: path.join(PAYLOAD, name), to: path.join(t.proxy, name), mode: name.endsWith(".mjs") ? "executable" : "file" });
  }
  for (const name of WRAPPER_FILES) {
    plan.push({ action: "backup-if-exists", from: path.join(t.localBin, name), backupDir });
    plan.push({ action: "write", from: path.join(WRAPPERS, name), to: path.join(t.localBin, name), mode: "cmd" });
  }
  ...
  plan.push({ action: "write-template", from: TASK_TEMPLATE, to: t.taskXml });
  if (opts.enableTask) plan.push({ action: "schtasks", command: `schtasks /Create /TN OpenRouterClaudeProxy /XML "${t.taskXml}" /F` });
  else plan.push({ action: "manual-start", command: path.join(t.localBin, "start-openrouter-claude-proxy.cmd") });
...
async function writeRendered(src, dest, t, mode) {
  ...
  await fs.writeFile(dest, mode === "cmd" ? renderCmd(text, t) : renderPayload(text, t));
}
...
async function applyInstall(opts, t, installPlan, apiKey) {
  await fs.mkdir(installPlan.backupDir, { recursive: true });
  for (const item of installPlan.plan.filter((x) => x.action === "backup-if-exists")) await backup(item.from, item.backupDir, t.home);
  for (const name of REQUIRED_PROXY_FILES) await writeRendered(path.join(PAYLOAD, name), path.join(t.proxy, name), t, "payload");
  for (const name of WRAPPER_FILES) await writeRendered(path.join(WRAPPERS, name), path.join(t.localBin, name), t, "cmd");
  ...
  await fs.writeFile(t.vscodeSettings, JSON.stringify({
    ...
    "claudeCode.claudeProcessWrapper": path.join(t.localBin, "claude-full.cmd"),
    "claudeCode.disableLoginPrompt": true,
    "claudeCode.preferredLocation": "panel",
  }, null, 2) + "\n");

  await writeRendered(TASK_TEMPLATE, t.taskXml, t, "cmd");
  if (opts.enableTask) {
    const out = await run("schtasks", ["/Create", "/TN", "OpenRouterClaudeProxy", "/XML", t.taskXml, "/F"], { timeout: 60_000 });
    if (!out.ok) throw new Error(`schtasks failed: ${out.stderr}`);
  }
  return { backupDir: installPlan.backupDir, taskEnabled: opts.enableTask };
}
...
async function main() {
  ...
  const t = targets(home, opts.fakeHome);
  ...
  if (!opts.yes) {
    const rl = readline.createInterface({ input, output });
    const answer = await rl.question("Proceed with Windows beta install? Type YES: ");
    rl.close();
